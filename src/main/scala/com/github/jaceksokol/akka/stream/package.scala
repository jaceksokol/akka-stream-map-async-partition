...
package object stream {
  ...
  private def extractPartitionWithCtx[In, Ctx](extract: In => Partition)(tuple: (In, Ctx)): Partition =
    extract(tuple._1)
  ...
  implicit class SourceExtension[In, +Mat](source: Source[In, Mat]) {
    def mapAsyncPartition[T](parallelism: Int, bufferSize: Int = DefaultBufferSize)(extractPartition: In => Partition)(
      f: In => Future[T]
    ): Source[T, Mat] =
      source.via(new MapAsyncPartition[In, T](parallelism, bufferSize, extractPartition, f))
  }
  ...
  implicit class SourceWithContextExtension[In, +Ctx, +Mat](flow: SourceWithContext[In, Ctx, Mat]) {
    def mapAsyncPartition[T](parallelism: Int, bufferSize: Int = DefaultBufferSize)(extractPartition: In => Partition)(
      ...
      flow.via(
        new MapAsyncPartition[(In, Ctx), (T, Ctx)](
          parallelism,
          bufferSize,
  ...
  implicit class FlowExtension[In, +Out, +Mat](flow: Flow[In, Out, Mat]) {
    def mapAsyncPartition[T](parallelism: Int, bufferSize: Int = DefaultBufferSize)(extractPartition: Out => Partition)(
      f: Out => Future[T]
    ): Flow[In, T, Mat] =
      flow.via(new MapAsyncPartition[Out, T](parallelism, bufferSize, extractPartition, f))
  }
  ...
  implicit class FlowWithContextExtension[In, +Out, Ctx, +Mat](flow: FlowWithContext[In, Ctx, Out, Ctx, Mat]) {
    def mapAsyncPartition[T](parallelism: Int, bufferSize: Int = DefaultBufferSize)(extractPartition: Out => Partition)(
      ...
      flow.via(
        new MapAsyncPartition[(Out, Ctx), (T, Ctx)](
          parallelism,
          bufferSize,
          extractPartitionWithCtx(extractPartition),
          fWithCtx(f)
